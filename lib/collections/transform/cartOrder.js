...
export const cartOrderTransform = {
   ...
   */
  getShippingTotal() {
    const shipments = this.shipping;
    // Use shipping that the request status is successful
    const successfulShipping = shipments.filter((shipment) => {
      return shipment.shipmentQuotesQueryStatus.requestStatus === "success";
    });
    if (successfulShipping) {
    // loop through the cart.shipping, sum shipments.
      const rate = getSummary(successfulShipping, ["shipmentMethod", "rate"]);
      const handling = getSummary(successfulShipping, ["shipmentMethod", "handling"]);
